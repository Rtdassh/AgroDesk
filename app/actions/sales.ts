...
export async function getProductosForSale() {
  ...
  return data.map(p => {
    const stock_disponible = p.lotes ? p.lotes.reduce((sum: number, l: any) => sum + (l.stock_actual || 0), 0) : 0;
    return {
...
export async function createSale(data: {
  ...
  // In production Supabase, it's highly recommended to use a Postgres function for this.
  ...
  try {
    ...
    // 3. Update Stock (Lotes) - Simple approach taking from first available lote
    for (const detalle of data.detalles) {
      // Get lotes for this product ordered by expiration or id
      ...
      if (remainingToDeduct > 0) {
         // This means stock changed concurrently and isn't enough anymore
         // Ideally we would rollback the whole transaction here using RPC
         console.warn(`Stock insufficient for product ${detalle.id_producto} during deduction`);
      }
    }

    // 4. Generate Comprobante
    // Getting last correlativo to generate new one
    ...
    revalidatePath("/inventario"); // Revalidate inventory as stock changed
